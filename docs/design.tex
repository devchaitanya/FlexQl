...
\rhead{\small\textit{devchaitanya}}
\cfoot{\thepage}
...
\begin{lstlisting}[language=C++]
class StringArena {
    static constexpr size_t BLOCK = 512 * 1024;  // 512 KB slabs
...
StringArena slabs & Per-string \texttt{std::string} &
  All data in $\sim$2 $\times$ 512\,KB blocks; hot strings stay in L2/L3 cache \\
\addlinespace
...
\textbf{Hardware:} Intel Core i7-10700K @ 3.8\,GHz, 16\,GB RAM, Ubuntu 24.04 LTS.
\textbf{Build:} \texttt{-O3 -march=native -std=c++17}.

\subsection{INSERT Benchmark}

Schema: \texttt{BIG\_USERS(ID DECIMAL, NAME VARCHAR(64), EMAIL VARCHAR(64), BALANCE DECIMAL, EXPIRES\_AT DECIMAL)} — 5 columns, no primary key, batch size 5,000.

\begin{table}[H]
\centering
\begin{tabular}{lrrl}
\toprule
\textbf{Dataset} & \textbf{Elapsed} & \textbf{Throughput} & \textbf{Notes} \\
\midrule
1\,M rows & $\sim$610\,ms & $\sim$1.64\,M rows/sec & WAL overhead $\sim$55\,ms \\
5\,M rows & $\sim$3.28\,s & $\sim$1.52\,M rows/sec & WAL overhead $\sim$250\,ms \\
\bottomrule
\end{tabular}
\caption{INSERT benchmark results (with full WAL persistence)}
\end{table}

\subsection{SELECT Queries (after 1\,M-row insert)}
...
\midrule
\texttt{SELECT COUNT(*) FROM BIG\_USERS} & $\sim$210\,ms \\
\texttt{SELECT * WHERE ID = 500000} (PK lookup) & $\sim$81\,ms \\
\texttt{SELECT ... WHERE ID > 999990 ORDER BY ID DESC LIMIT 10} & $\sim$81\,ms \\
4 concurrent \texttt{SELECT COUNT(*)} & $\sim$325\,ms each \\
\bottomrule
\end{tabular}
\caption{SELECT query latencies on 1\,M-row table}
\end{table}

\texttt{COUNT(*)} performs a full table scan. PK equality lookups are $O(1)$ via the
flat hash map.
...
\begin{tabular}{lr}
\toprule
\textbf{Dataset} & \textbf{RSS} \\
\midrule
1\,M rows (5 columns) & $\sim$615\,MB \\
5\,M rows (5 columns) & $\sim$727\,MB \\
\bottomrule
\end{tabular}
\caption{Server resident set size}
\end{table}

\subsection{Unit Test Results}

\textbf{22 / 22} benchmark unit tests pass on every run. Tests cover: single-row
\texttt{SELECT}, filtered \texttt{SELECT} with \texttt{ORDER BY}, empty result sets,
multi-table \texttt{INNER JOIN}, \texttt{JOIN} with no matches, invalid column error,
and missing table error.
...
\begin{lstlisting}[language=bash]
./bench/benchmark 1000000    # 1M-row insert + unit tests
./bench/benchmark --unit-test  # unit tests only
./bench/benchmark 5000000    # 5M rows
\end{lstlisting}
