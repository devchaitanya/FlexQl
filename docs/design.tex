...
\subsection{WHERE Predicates (Compound)}

Operators: \texttt{=}, \texttt{!=}, \texttt{<}, \texttt{<=}, \texttt{>}, \texttt{>=},
\texttt{LIKE} (\texttt{\%} and \texttt{\_} wildcards), \texttt{BETWEEN v AND v},
\texttt{IN (v, ...)}, \texttt{IS NULL}, \texttt{IS NOT NULL},
combined with \texttt{AND}, \texttt{OR}, \texttt{NOT}, and nested parentheses.

The predicate is represented as a tree (\texttt{LEAF / AND\_NODE / OR\_NODE / NOT\_NODE})
evaluated recursively during a table scan.
...
\subsection{Predicate Tree for Compound WHERE}

Rather than evaluating WHERE clauses as a flat list of conditions, the parser builds a
tree of \texttt{Predicate} nodes (\texttt{LEAF}, \texttt{AND\_NODE}, \texttt{OR\_NODE},
\texttt{NOT\_NODE}). This supports arbitrarily nested Boolean logic such as
\texttt{WHERE (A > 1 OR B < 5) AND NOT C = 3}. The tree is evaluated recursively during
the table scan --- no intermediate materialisation of row sets.
...
\texttt{SELECT * WHERE DEPT='ENG' AND SALARY>80000}          & 34,986  & 109\,ms  \\
\texttt{SELECT * WHERE DEPT='HR' OR DEPT='SALES'}            & 250,000 & 348\,ms  \\
...
\texttt{UPDATE} filtered (\texttt{DEPT='QA' AND SALARY<50000})   & 73\,ms  \\
\texttt{DELETE} 100 rows (\texttt{WHERE ID<=100})                 & 143\,ms \\
\texttt{DELETE} filtered (\texttt{DEPT='LEGAL' AND SALARY<40000}) & 72\,ms  \\
\bottomrule
\end{tabular}
