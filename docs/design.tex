...
\textbf{Build:} \texttt{-O3 -march=native -std=c++17 -pthread}.
...
\midrule
1\,M rows  & 280\,ms   & 3.57\,M rows/sec \\
10\,M rows & 3,349\,ms & 2.99\,M rows/sec \\
\bottomrule
\end{tabular}
\caption{INSERT benchmark (batch size 5\,K--25\,K)}
\end{table}

\subsection{SELECT and Aggregate Queries (1\,M rows)}
...
\midrule
\texttt{SELECT COUNT(*)}                                     & 1       & 1.2\,ms  \\
\texttt{SELECT *} (full scan)                                & 1\,M    & 608\,ms  \\
\texttt{SELECT * WHERE ID = 500000}                          & 1       & 0.8\,ms  \\
\texttt{SELECT * WHERE SALARY > 90000}                       & 139,986 & 83\,ms   \\
\texttt{SELECT * WHERE SALARY BETWEEN 50000 AND 60000}       & 140,015 & 84\,ms   \\
\texttt{SELECT * WHERE DEPT IN ('ENG','HR','FIN')}           & 375,000 & 324\,ms  \\
\texttt{SELECT * WHERE NAME LIKE 'user1\%'}                  & 111,112 & 113\,ms  \\
\texttt{SELECT * ORDER BY SALARY DESC LIMIT 10}              & 10      & 8.5\,ms  \\
\texttt{SELECT DISTINCT DEPT}                                & 8       & 113\,ms  \\
\texttt{SELECT SUM/AVG/MIN/MAX(SALARY)}                      & 1       & 3.9--4.2\,ms \\
\texttt{GROUP BY DEPT} (8 groups)                            & 8       & 79\,ms   \\
\bottomrule
\end{tabular}
\caption{SELECT and aggregate query latencies on 1\,M-row table}
\end{table}

\subsection{SELECT Queries (10\,M rows)}
...
\textbf{Query} & \textbf{Rows} & \textbf{Elapsed} \\
\midrule
\texttt{SELECT COUNT(*)}                                     & 1         & 11\,ms     \\
\texttt{SELECT * WHERE ID = 5000000}                         & 1         & 5.3\,ms    \\
\texttt{SELECT * WHERE BALANCE > 10000}                      & 999,000   & 526\,ms    \\
\texttt{SELECT * WHERE BALANCE BETWEEN 5000 AND 6000}        & 1,001,000 & 529\,ms    \\
\texttt{SELECT * WHERE ID IN (4 values)}                     & 4         & 8.3\,ms    \\
\texttt{SELECT * WHERE NAME LIKE 'user1\%'}                  & 1,111,112 & 1,035\,ms  \\
\texttt{SELECT * ORDER BY BALANCE DESC LIMIT 10}             & 10        & 80\,ms     \\
\texttt{SELECT SUM/AVG/MIN/MAX(BALANCE)}                     & 1         & 27--30\,ms \\
\bottomrule
\end{tabular}
\caption{SELECT query latencies on 10\,M-row table (3 columns)}
\end{table}

Aggregates on 10\,M rows complete in 27--30\,ms via the native \texttt{int64\_t}
fast path.  Point lookups remain under 10\,ms at 10$\times$ scale.
...
\textbf{Query} & \textbf{Rows} & \textbf{Elapsed} \\
\midrule
\texttt{INNER JOIN} (1\,M $\times$ 8, all match)      & 1\,M    & 574\,ms \\
\texttt{INNER JOIN + WHERE SALARY>90000}               & 139,986 & 312\,ms \\
\texttt{LEFT JOIN} (1\,M $\times$ 8)                   & 1\,M    & 692\,ms \\
\bottomrule
\end{tabular}
\caption{JOIN query latencies (BENCH 1\,M rows $\times$ DEPARTMENTS 8 rows)}
\end{table}

\subsection{UPDATE, DELETE, and Cache}
...
\midrule
\texttt{UPDATE} single row (\texttt{WHERE ID=1})                 & 49\,ms  \\
\texttt{UPDATE} 1,000 rows (\texttt{WHERE ID<=1000})             & 76\,ms  \\
\texttt{DELETE} 100 rows (\texttt{WHERE ID<=100})                 & 80\,ms  \\
LRU cache cold $\to$ warm (point query)                          & 0.82\,ms $\to$ 0.02\,ms ($37\times$) \\
\bottomrule
\end{tabular}
\caption{UPDATE / DELETE / Cache latencies on 1\,M-row table}
\end{table}
...
\toprule
\textbf{Benchmark} & \textbf{Threads} & \textbf{Avg Latency} & \textbf{Throughput} \\
\midrule
\multicolumn{4}{l}{\textit{Concurrent reads (point queries, 1\,M rows)}} \\
 & 1 & 0.67\,ms & 20 queries / 2\,ms \\
 & 4 & 0.64\,ms & 80 queries / 2\,ms \\
 & 8 & 0.40\,ms & 160 queries / 3\,ms \\
\midrule
\multicolumn{4}{l}{\textit{Mixed read/write (3-second run, 1\,M rows)}} \\
4\,R + 1\,W  &  5 & --- & 239 ops/sec \\
4\,R + 4\,W  &  8 & --- & 291 ops/sec \\
\midrule
\multicolumn{4}{l}{\textit{Concurrent INSERTs (separate tables, 50\,K rows/thread)}} \\
 & 1 &  --- & 5.0\,M rows/sec \\
 & 4 &  --- & 18.2\,M rows/sec \\
 & 8 &  --- & 16.7\,M rows/sec \\
\bottomrule
\end{tabular}
\caption{Concurrency benchmarks --- \texttt{shared\_lock} for reads, per-table
\texttt{unique\_lock} for writes}
\end{table}
...
\begin{tabular}{L{8cm} r r}
\toprule
\textbf{Operation} & \textbf{Rows} & \textbf{Elapsed} \\
\midrule
PK INSERT 100\,K rows                   & 100,000 & 36\,ms  \\
PK point lookup (\texttt{ID=50000})      & 1       & $<$1\,ms \\
PK IN lookup (4 keys)                    & 4       & $<$1\,ms \\
PK duplicate INSERT rejection            & ---     & $<$1\,ms \\
\bottomrule
\end{tabular}
\caption{Primary key hash-map operations (flat open-addressing FNV-64)}
\end{table}

\subsection{Memory and Tests}

Server RSS: 520\,MB after 1\,M-row seed, 788\,MB after full benchmark suite.
\textbf{564 / 564} unit tests pass (\texttt{make test}).
